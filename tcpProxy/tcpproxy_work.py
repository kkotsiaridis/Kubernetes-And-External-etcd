...
def start_proxy_thread(local_socket, args, in_modules, out_modules,target_ips,no_thread):
    ...
    # passing it on.
    print("========================new thread created- %d =============================================" %(no_thread))
    in_modules_list = []
    out_modules_list = []

    number_of_ips = len(target_ips)

    for i in range(number_of_ips):
        in_modules_list.append(in_modules)
        out_modules_list.append(out_modules)

    remote_socket = []
    for i in range(number_of_ips):
        remote_socket.append(socks.socksocket())
    ...
    try:
        for i in range(number_of_ips):
            remote_socket[i].connect((target_ips[i], args.target_port))
            vprint('Connected to %s:%d' % remote_socket[i].getpeername(), args.verbose)
            log(args.logfile, 'Connected to %s:%d' % remote_socket[i].getpeername())
    except socket.error as serr:
        if serr.errno == errno.ECONNREFUSED:
            for i in range(number_of_ips):
                for s in [remote_socket[i], local_socket]:
                            s.close()
                print(f'{time.strftime("%Y%m%d-%H%M%S")}, {target_ips[i]}:{args.target_port}- Connection refused')
                log(args.logfile, f'{time.strftime("%Y%m%d-%H%M%S")}, {target_ips[i]}:{args.target_port}- Connection refused')
                return None
        elif serr.errno == errno.ETIMEDOUT:
            for i in range(number_of_ips):
                for s in [remote_socket[i], local_socket]:
                        s.close()
                print(f'{time.strftime("%Y%m%d-%H%M%S")}, {target_ips[i]}:{args.target_port}- Connection timed out')
                log(args.logfile, f'{time.strftime("%Y%m%d-%H%M%S")}, {target_ips[i]}:{args.target_port}- Connection timed out')
                return None
        else:
            for i in range(number_of_ips):
                for s in [remote_socket[i], local_socket]:
    ...
    try:
        for i in range(number_of_ips):
            update_module_hosts(out_modules_list[i], local_socket.getpeername(), remote_socket[i].getpeername())
            update_module_hosts(in_modules_list[i], remote_socket[i].getpeername(), local_socket.getpeername())
    except socket.error as serr:
    ...
    counter = 0
    while running:
        read_sockets, _, _ = select.select([remote_socket[0],remote_socket[1],remote_socket[2], local_socket], [], [])
        #print("%s ------> no=%d - cntr=%d"%(read_sockets, no_thread,counter))
        counter +=1
        if starttls(args, local_socket, read_sockets):
            try:
                ssl_sockets = enable_ssl(args, remote_socket[etcd_leader], local_socket)
        ...
        for sock in read_sockets:
            try:
                ...
                if serr.errno == errno.ENOTCONN:
                    ...
                    # I don't yet understand how this error can happen, but if it happens I'll just shut down the thread
                    # the connection is not in a useful state anymore
                    for s in [remote_socket[etcd_leader], local_socket]:
                        s.close()
                    running = False
                    break
                else:
            ...
            if sock == local_socket:
                if len(data):
                    ...
                    if out_modules_list[etcd_leader]  is not None:
                        data = handle_data(data, out_modules_list[etcd_leader] ,
                                           args.no_chain_modules,
                                           False,  # incoming data?
                                           args.verbose)
                    remote_socket[etcd_leader].send(data.encode() if isinstance(data, str) else data)
                else:
                    ...
                    remote_socket[etcd_leader].close()
                    running = False
                    break
            elif sock == remote_socket[etcd_leader]:
                if len(data):
                    log(args.logfile, b'> > > in\n' + data)
                    if in_modules_list[etcd_leader] is not None:
                        data = handle_data(data, in_modules_list[etcd_leader],
                                           args.no_chain_modules,
                                           True,  # incoming data?
                                           args.verbose)
                    ...
                    running = False
                    break
    #print("++++++++++close thread %d +++++++++++++++" %(no_thread))                  



def start_time_thread(args,target_ips):
    global etcd_leader
    while(True):
        etcdctl = os.popen("ETCDCTL_API=3 etcdctl \
           --endpoints=http://"+target_ips[0]+":2379,http://"+target_ips[1]+":2379,http://"+target_ips[2]+":2379 \
           endpoint status").read().split(' ')
        #print(etcdctl[5],etcdctl[15],etcdctl[25])
        etcd_leader = 0
        if(etcdctl[15] == 'true,'):
            etcd_leader = 1
        elif(etcdctl[25] == 'true,'):
            etcd_leader = 2
        #print("leader is %d"%(etcd_leader))
        time.sleep(2)
...
def main():
    ...
    target_ips_tmp = args.target_ip.split(',')
    target_ips = sorted(target_ips_tmp)
    #print(target_ips)

    args.target_ip = target_ips[0]
    ...
    no_thread = 0
    time_thread = threading.Thread(target=start_time_thread,args=(args,target_ips))
    time_thread.start()
    try:
        while True:
